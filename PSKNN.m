function arcv = PSKNN( func_num, dim, seed, psr)
...
while fe < maxfe 
    ...
    for k=1:nc/2
        ...
        temp_predict_y = zeros(1, 2*psr);
        for i = 1:2*psr
            % Pre-selection
            train = get_traing_data(arcv, knn, temp(:, i)');
            dis = pdist2(temp(:, i)', train.x)';
            if sum(dis <= 0.1 )
                
                [~, position] = min(dis);
                temp_predict_y(i) = train.y(position);
                ...
                a = sum(train.y .* (d.^2));
                b = sum((d.^2));
                temp_predict_y(i) = a/b;
                
            end          
            
        end
        [~, index] = sort(temp_predict_y);
